...
- (void)fetchBuildings:(CDVInvokedUrlCommand*)command
{
    ...
    [[SITCommunicationManager sharedManager] fetchBuildingsWithOptions:options success:^(NSDictionary *mapping) {
        ...
        if (buildings.count == 0) {
            ...
            pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"There are no buildings on the account. Please go to dashboard http://api.situm.com and learn more about the first step with Situm technology"];
        }
        else {
...
- (void)fetchFloorsFromBuilding:(CDVInvokedUrlCommand*)command
{
    ...
    [[SITCommunicationManager sharedManager] fetchFloorsForBuilding:buildingId withOptions:nil success:^(NSDictionary *mapping) {
        ...
        if (floors.count == 0) {
            ...
            pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"The selected building does not have floors. Correct that on http://api.situm.com"];
        } else {
            if (IS_LOG_ENABLED) {
